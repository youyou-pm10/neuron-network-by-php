...
*/
    ...
    public function feedback($neurons, $rate)
    {
        foreach ($neurons as $neuron){
            if(!empty($neuron->axons)){
                foreach ($neuron->axons as $axon){
                    $fixWeight = $neuron->derivative($neuron->input) * $neuron->input *  $neuron->inputLoss[$axon->name] * $rate;
                    $fixBias = $neuron->derivative($neuron->input) *  $neuron->inputLoss[$axon->name] * $rate;
                    $weight = $this->findWeight($neuron, $axon)['weight'] - $fixWeight;
                    $bias = $this->findWeight($neuron, $axon)['bias'] - $fixBias;
